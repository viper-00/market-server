...
func (m *MService) IsUserExistByEmail(email string) (bool, error) {
	var user model.User
...
func (m *MService) IsUserExistByAddressAndChain(address string, chainId int) (bool, error) {
	var user model.User
	err := global.MARKET_DB.Where("address = ? AND chain_id = ? AND status = 1", address, chainId).First(&user).Error
...
func (m *MService) UserRegister(info request.UserRegister) error {
	if info.Email != "" {
		...
		isExist, errExist := m.IsUserExistByEmail(info.Email)
		if isExist {
		...
		if !constant.IsAddressSupport(info.ChainId, address) {
			return fmt.Errorf("do not support the wallet address: id: %d, address: %s", info.ChainId, info.Address)
		}

		isExist, errExist := m.IsUserExistByAddressAndChain(info.Address, info.ChainId)
		if isExist {
			return errors.New("user already exists")
		} else if errExist == nil && !isExist {
			// initialze account
			if err := m.InitializeAccount(info.ChainId, "", info.Address); err != nil {
				global.MARKET_LOG.Error(err.Error())
				return err
			}
			return nil
...
func (n *MService) UserLoginByWallet(info request.UserLoginByWallet) (user response.User, err error) {
	if info.Address != "" && info.ChainId != 0 {
		// login by contract
		var modelUser model.User
		err = global.MARKET_DB.Where("address = ? AND chain_id = ? AND status = 1", info.Address, info.ChainId).First(&modelUser).Error
		if err != nil {
			global.MARKET_LOG.Error(err.Error())
			return
		}
		...
		if _, err = global.MARKET_REDIS.Set(context.Background(), user.Auth, info.Address, time.Hour*24).Result(); err != nil {
			global.MARKET_LOG.Error(err.Error())
			return
		}
...
func (m *MService) InitializeAccount(chainId int, email, connectAddress string) (err error) {
	var (
		address    string
		privateKey string
		way        uint
